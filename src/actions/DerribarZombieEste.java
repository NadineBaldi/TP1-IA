...
public class DerribarZombieEste extends SearchAction {
	...
	public SearchBasedAgentState execute(SearchBasedAgentState s) {
		...
		if (estadoPlanta.getCantSolesDisponibles() > 0) { //ejecutar la accion solo si el agente tiene soles disponibles (CONDICION DE PARADA)
			...
			List<Zombie> zombiesPercibidos = siguienteEstado.getZombiesPercibidos();
			
			if (ubicacion.y < 8) { //Chequeo que el agente no se encuentre en la ultima columna (sobre el borde derecho)
				...
				if (celdaAdyacente.hayZombie()) { //si hay zombie en la celda adyacente
						...
						for(int i=0;i<zombiesPercibidos.size() && bandera;i++) {
								...
								if((zombiesPercibidos.get(i).getUbicacionZombie().x==ubicacion.x) &&
								  (zombiesPercibidos.get(i).getUbicacionZombie().y==ubicacion.y+1)) {
									...
									if (zombiesPercibidos.get(i).getCosteDeSoles() < siguienteEstado.getCantSolesDisponibles()) {
										index = i;
									}
									bandera=false;
								}
						...
						if (index != -1) {
							...
							siguienteEstado.setCantSolesDisponibles(siguienteEstado.getCantSolesDisponibles()-zombiesPercibidos.get(index).getCosteDeSoles());
