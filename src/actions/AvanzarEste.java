		...
		if (estadoPlanta.getCantSolesDisponibles() > 0) { //ejecutar la accion solo si el agente tiene soles disponibles (CONDICION DE PARADA)
			...
			if(posicion.y < 8) {
				...
				if (mapa[posicion.x][posicion.y+1].getTipo() == TipoEnum.GIRASOL) {
					...
					if (mapa[posicion.x][posicion.y+1].getTipo() == TipoEnum.ZOMBIE) { // si en la proxima celda hay un zombie
						
						Zombie zombieEncontrado = siguienteEstado.buscarZombie(posicion.x, posicion.y+1);
						if(zombieEncontrado != null) {
							...
							if (costoDeMatarZombie < siguienteEstado.getCantSolesDisponibles()) { 
								mapa[posicion.x][posicion.y+1].setTipo(TipoEnum.PLANTA_AGENTE);	//reemplazo la celda con zombie por una celda con agente
		...
		if (estadoPlanta.getCantSolesDisponibles() > 0) { //ejecutar la accion solo si el agente tiene soles disponibles (CONDICION DE PARADA)
			...
			if(columna < 8) {
				...
				if (mapaAgente[posicion.x][posicion.y+1].getTipo() == TipoEnum.GIRASOL) {
					...
					if (mapaAgente[posicion.x][posicion.y+1].getTipo() == TipoEnum.ZOMBIE) { // si en la proxima celda hay un zombie
						
						Zombie zombieEncontrado = estadoPlanta.buscarZombie(posicion.x, posicion.y+1);
						int costoDeMatarZombie = zombieEncontrado.getCosteDeSoles()*2; //Cuesta el doble matar a un zombie si la planta se mueve a su celda
						
						//Si la planta tiene mas soles que la cantidad necesaria p matar al zombie multiplicado por dos
						if (costoDeMatarZombie < estadoPlanta.getCantSolesDisponibles()) { 
							mapaAgente[posicion.x][posicion.y+1].setTipo(TipoEnum.PLANTA_AGENTE);	//reemplazo la celda con zombie por una celda con agente
				...
				// Actualizo el mapa del ambiente poniendo en vacia su posicion anterior (o si hay un girasol, pongo el girasol)
				// y colocando al agente en la nueva posicion
				...
				if (mapaAmbiente[posicion.x][posicion.y+1].getTipo() == TipoEnum.GIRASOL) {
					...
					if (mapaAmbiente[posicion.x][posicion.y+1].getTipo() == TipoEnum.ZOMBIE) { // si en la proxima celda hay un zombie
						...
						//Si la planta tiene mas soles que la cantidad necesaria p matar al zombie multiplicado por dos
						if (costoDeMatarZombie < estadoPlanta.getCantSolesDisponibles()) { 
							mapaAmbiente[posicion.x][posicion.y+1].setTipo(TipoEnum.PLANTA_AGENTE);	//reemplazo la celda con zombie por una celda con agente
							estadoPlanta.setCantSolesDisponibles(estadoPlanta.getCantSolesDisponibles() - costoDeMatarZombie); //le resto los soles sacados x el zombie
							estadoPlanta.getZombiesPercibidos().remove(zombieEncontrado); //elimino de la lista de zombies percibidos al zombie matado
							estadoPlanta.setCantZombies(estadoPlanta.getCantZombies() - 1);
						} else { //no le alcanzan los soles y muere la planta
