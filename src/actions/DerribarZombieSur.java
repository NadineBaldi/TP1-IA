...
public class DerribarZombieSur extends SearchAction{
	...
	public SearchBasedAgentState execute(SearchBasedAgentState s) {
		...
		if (estadoPlanta.getCantSolesDisponibles() > 0) { //ejecutar la accion solo si el agente tiene soles disponibles (CONDICION DE PARADA)
			...
			if (ubicacion.x < 4) { //Chequeo que el agente no se encuentre en la ultima fila (sobre el borde inferior) y quiera matar a alguien de la siguiente fila
				
				Celda celdaAdyacente = mapa[ubicacion.x+1][ubicacion.y];
				...
				if (celdaAdyacente.hayZombie()) { //si hay zombie en la celda adyacente
						...
						for(int i=0;i<zombiesPercibidos.size() && bandera;i++) {
							...
							if((zombiesPercibidos.get(i).getUbicacionZombie().x==ubicacion.x+1) &&
							  (zombiesPercibidos.get(i).getUbicacionZombie().y==ubicacion.y)) {
	...
	public EnvironmentState execute(AgentState ast, EnvironmentState est) {
		...
		if (estadoPlanta.getCantSolesDisponibles() > 0) { //ejecutar la accion solo si el agente tiene soles disponibles (CONDICION DE PARADA)
			...
			if (ubicacion.x < 4) { //Chequeo que el agente no se encuentre en la ultima fila (sobre el borde inferior) y quiera matar a alguien de la siguiente fila
				...
				Celda celdaAdyacente = mapaAgente[ubicacion.x+1][ubicacion.y];
				...
				if (celdaAdyacente.hayZombie()) { //si hay zombie en la celda adyacente
						...
						for(int j=0;j<zombiesPercibidos.size() && bandera;j++) {
							...
							if((zombiesPercibidos.get(j).getUbicacionZombie().x==ubicacion.x+1) &&
							  (zombiesPercibidos.get(j).getUbicacionZombie().y==ubicacion.y)) {
						...
						if (index1 != -1) {
							...
							mapaAmbiente[ubicacion.x+1][ubicacion.y].setTipo(TipoEnum.VACIO);
							estadoPlanta.setCantZombies(estadoPlanta.getCantZombies()-1);
						...
						if (index2 != -1) {
							...
							mapaAgente[ubicacion.x+1][ubicacion.y].setTipo(TipoEnum.VACIO);
							estadoPlanta.setCantZombies(estadoPlanta.getCantZombies()-1);
